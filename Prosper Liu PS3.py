...
def trajectory (x0,y0,v,theta,g = 9.8, npts = 1000):
    ...
    vy0=np.sin(theta)*v
    vx0=np.cos(theta)*v
    tf=(vy0/g) + np.sqrt((vy0/g) ** 2 - 2 * (y0/g))
    t=np.linspace(0, tf, npts)
    y=-0.5*g*t**2+y0+(t*vy0)
    x=(vx0*t)+x0
    return x, y
...
def firstInBox (x,y,box):
    ...
    for j in range(len(x)):
        if (box[2] <= y[j] <= box[3]) and (box[0] <= x[j] <= box[1]):
            return j
    return -1
...
def tankShot (targetBox, obstacleBox, x0, y0, v, theta, g = 9.8):
    ...
    hitone = firstInBox(x, y, obstacleBox)
    hittwo = firstInBox(x, y, targetBox)
    if hittwo == -1:
        if hitone >= 0:
...
def oneTurn (tank1box, tank2box, obstacleBox, playerNum, g = 9.8):   
    ...
    if playerNum ==1:
        y0 = (tank1box[2] + tank1box[3])*0.5
        x0 = (tank1box[0] + tank1box[1])*0.5
        velocity = getNumberInput('Please enter velocity, player 1:')
        angdeg = getNumberInput('Please enter angle (degrees), player 1:')
        drawBoard(tank1box, tank2box, obstacleBox, playerNum)
        tankShot(tank2box, obstacleBox, x0, y0, velocity, angdeg)
        if tankShot(tank2box, obstacleBox, x0, y0, velocity, angdeg) == 1:
            return playerNum
        else:
            return 0
    else:
        y0 = (tank1box[2] + tank1box[3])*0.5
        x0 = (tank1box[0] + tank1box[1])*0.5
        velocity = getNumberInput('Please enter velocity, player 2:')
        angdeg = getNumberInput('Please enter angle (degrees), player 1:')
        drawBoard(tank1box, tank2box, obstacleBox, playerNum)
        tankShot(tank1box, obstacleBox, x0, y0, velocity, angdeg)
        if tankShot(tank1box, obstacleBox, x0, y0, velocity, angdeg) == 1:
            return playerNum
        else:
            return 0
